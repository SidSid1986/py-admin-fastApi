...
async def poll_visitor_list(
    ...
    first_snapshot = get_current_list_snapshot()

    # 先查 → 有变立刻返回 → 没有才等待
    for _ in range(max_wait):
        current_snapshot = get_current_list_snapshot()
        if current_snapshot != first_snapshot:
            break
        await asyncio.sleep(check_interval)
