...
class SeatManager:
    def __init__(self, total_seats=2):
        ...
        self.last_allocated_seat = 0

        # 页面活着的超时时间：前端心跳断了就释放
        self.HEARTBEAT_TIMEOUT = 30

        # 聊天空闲超时时间：页面还开着但长时间没互动也释放
        self.INACTIVE_TIMEOUT = 600  # 10 分钟
    ...
    def _is_heartbeat_alive(self, info):
        return info["busy"] and (time.time() - info["heartbeat"]) <= self.HEARTBEAT_TIMEOUT

    def _is_chat_active(self, info):
        return info["busy"] and (time.time() - info["last_active_time"]) <= self.INACTIVE_TIMEOUT

    def _is_valid_seat(self, info):
        return self._is_heartbeat_alive(info) and self._is_chat_active(info)

    def get_active_count(self):
        """获取真正有效的座席数量"""
        active_count = 0
        for info in self.seats.values():
            if self._is_valid_seat(info):
                active_count += 1
        return active_count
    ...
    def clean_expired(self):
        ...
        for seat_id, info in self.seats.items():
            ...
            heartbeat_expired = (now - info["heartbeat"]) > self.HEARTBEAT_TIMEOUT
            inactive_expired = (now - info["last_active_time"]) > self.INACTIVE_TIMEOUT

            if heartbeat_expired or inactive_expired:
                reason = []
                if heartbeat_expired:
                    reason.append("heartbeat timeout")
                if inactive_expired:
                    reason.append("inactive timeout")

                print(f"[DEBUG] 清理过期座席 {seat_id}，访客ID: {info['visitor_id']}，原因: {', '.join(reason)}")
                self.seats[seat_id] = {
                    "busy": False,
                    "visitor_id": None,
                    "heartbeat": 0,
    ...
    def allocate_seat(self, visitor_id):
        ...
        with self.lock:
            ...
            # 如果当前访客已有有效座席，直接复用并刷新时间
            for seat_id, info in self.seats.items():
                if info["visitor_id"] == visitor_id and self._is_valid_seat(info):
                    now = time.time()
                    self.seats[seat_id]["heartbeat"] = now
                    self.seats[seat_id]["last_active_time"] = now
                    self.last_allocated_seat = seat_id
                    print(f"[DEBUG] 访客 {visitor_id} 已在使用座席 {seat_id}，刷新状态")
                    return {"code": 200, "agent": seat_id}

            active_count = self.get_active_count()
            print(f"[DEBUG] 当前有效座席数: {active_count}, 最大限制: {self.total_seats}")
    ...
    def release_seat_if_owned(self, seat_id, visitor_id):
        """只有当前访客仍然占用该座席时才允许释放"""
        with self.lock:
            self.clean_expired()
            info = self.seats.get(seat_id)
            if not info:
                return False
            if info["visitor_id"] != visitor_id or not self._is_valid_seat(info):
                print(f"[DEBUG] 拒绝释放座席 {seat_id}，当前访客: {info['visitor_id']}，请求访客: {visitor_id}")
                return False
            ...
            print(f"[DEBUG] 访客 {visitor_id} 释放了座席 {seat_id}")
            return True
    ...
    def touch_activity(self, visitor_id):
        """更新真实聊天活跃时间"""
        with self.lock:
            self.clean_expired()
            for seat_id, info in self.seats.items():
                if info["visitor_id"] == visitor_id and self._is_valid_seat(info):
                    ...
                    self.seats[seat_id]["heartbeat"] = now
                    self.seats[seat_id]["last_active_time"] = now
                    print(f"[DEBUG] 更新访客 {visitor_id} 在座席 {seat_id} 的聊天活跃时间")
                    return seat_id
        return None

    def get_visitor_seat(self, visitor_id):
        """获取访客当前仍然有效的座席"""
        with self.lock:
            self.clean_expired()
            for seat_id, info in self.seats.items():
...
def get_chat_manager(chat_type: str):
    if chat_type == "product":
        return product_seat_manager
    if chat_type == "maintain":
        return maintain_seat_manager
    return None
...
async def send_chat_to_wecom(
    ...
    manager = get_chat_manager(chat_type)
    if not manager:
        return {"code": 400, "msg": "无效的聊天类型"}

    current_seat = manager.touch_activity(visitor_id)
    if not current_seat:
        return {"code": 403, "msg": "当前会话未占用有效坐席，请重新打开咨询窗口"}
    ...
    try:
        ...
        agentid = WECOM_AGENT_ID if current_seat == 1 else WECOM_AGENT_ID2
        token = get_wecom_access_token() if current_seat == 1 else get_wecom_access_token2()
        ...
        return {"code": 200, "msg": "发送成功", "agent": current_seat}
...
async def wecom_callback(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if vid:
            product_seat_manager.touch_activity(vid)
            maintain_seat_manager.touch_activity(vid)
...
async def release_product_agent(data: ReleaseSeatRequest):
    released = product_seat_manager.release_seat_if_owned(data.agent, data.visitor_id)
    if not released:
        return {"code": 409, "msg": "座席已释放或不属于当前访客"}
    return {"code": 200}
...
async def release_maintain_agent(data: ReleaseSeatRequest):
    released = maintain_seat_manager.release_seat_if_owned(data.agent, data.visitor_id)
    if not released:
        return {"code": 409, "msg": "座席已释放或不属于当前访客"}
    return {"code": 200}
...
async def poll_heartbeat(visitor_id: str, chat_type: str):
    ...
    if not manager:
        return {"code": 400, "msg": "无效的聊天类型"}
    seat_id = manager.update_heartbeat(visitor_id)
    if not seat_id:
        return {"code": 403, "msg": "当前会话未占用有效坐席"}
    return {"code": 200, "agent": seat_id}

# ========================= 访客管理接口 =========================
@chat_router.get("/visitor-list", summary="获取所有在线访客（客服用）")
async def get_visitor_list():
    visitors = []

    # 产品咨询在线访客
    for seat_id, info in product_seat_manager.seats.items():
        if info["busy"] and info["visitor_id"]:
            visitors.append({
                "visitor_id": info["visitor_id"],
                "alias_name": get_friendly_visitor_id(info["visitor_id"]),
                "chat_type": "product",
                "unread": 0
            })

    # 维保咨询在线访客
    for seat_id, info in maintain_seat_manager.seats.items():
        if info["busy"] and info["visitor_id"]:
            visitors.append({
                "visitor_id": info["visitor_id"],
                "alias_name": get_friendly_visitor_id(info["visitor_id"]),
